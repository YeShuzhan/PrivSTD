...
def HB(x, eps, delta):
    ...
    BuildTree(Root, acc, newx, (0, 0), (l - 1, l - 1), b, eps * 1.0 / height, height, height, delta)
...
def BuildTree(Node, acc, X, start, end, b, epsilon, height, toth, delta):
    ''' Build the tree on X based on branching b and compute noisy count'''
    sigma = math.sqrt(2 * math.log(1.25 / delta)) / epsilon
    if height == 0:
        # gaussian noise
        nv = X[start[0]][start[1]] + numpy.random.normal(0, sigma)
        ...
        for i in range(len(newgridx) - 1):
            for j in range(len(newgridy) - 1):
                ...
                BuildTree(newN, acc, X, (x0, y0), (x1, y1), b, epsilon, height - 1, toth, delta)
        ...
        if height < toth:
            ...
            ntot = tot + numpy.random.normal(0, sigma)
